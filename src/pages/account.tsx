...
import { Product, Subscription } from 'src/interfaces/app.interface';
import { API_REQUEST } from 'src/services/api.service';
import moment from 'moment'

const Account = ({ subscription, subscPlan }: AccountProps) => {
    console.log(subscPlan);
        ...
        <>
            ...
            <main className='mx-auto max-w-6xl px-5 pt-24 pb-12 transition-all md:px-10'>
                ...
                <div className='mt-6 grid grid-cols-1 gap-x-4 border px-4 py-4 md:grid-cols-4 md:bordder-x-0 md:border-t md:border-b-0 md:pb-0'>
                    ...
                    <div className='col-span-2 font-medium'>{subscPlan?.name}</div>
...
export const getServerSideProps: GetServerSideProps<AccountProps> = async ({ req }) => {
    ...
    const subscPlans = await fetch(API_REQUEST.products_list).then(res => res.json());
    const subscPlan = subscPlans.products.data.length
        ? subscPlans.products.data.find((c: Product) => c.id === subscription.subscription.data[0]?.plan.product)
        : null
